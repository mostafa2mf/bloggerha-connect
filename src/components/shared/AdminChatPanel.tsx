...
const AdminChatPanel = ({ lang, chatLabel }: Props) => {
  ...
  const canSendMessages = Boolean(user && adminId && !isAdminPreview);
  ...
  useEffect(() => {
    ...
    if (!user) {
      dbg('init-skip', 'no authenticated user available');
      setMessages([]);
      setLoading(false);
      return;
    }
  ...
  const handleSend = async () => {
    if ((!newMsg.trim() && !attachPreview) || !adminId) return;
    if (!user || isAdminPreview) {
      toast.info(
        lang === 'fa'
          ? 'در پیش\u200cنمایش ادمین، ارسال واقعی پیام غیرفعال است. برای تست واقعی با حساب بلاگر یا کسب\u200cوکار وارد شوید.'
          : 'Live message sending is disabled in admin preview. Sign in with a real blogger or business account to test it.'
      );
      return;
    }
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center gap-3">
            ...
            <h3 className="text-base font-bold">
              {lang === 'fa' ? 'پیش\u200cنمایش چت تیام' : 'Tiam chat preview'}
            </h3>
            <p className="text-xs text-muted-foreground max-w-xs leading-6">
              {lang === 'fa'
                ? 'در این حالت فقط ظاهر چت را می\u200cبینید. برای ارسال و دریافت واقعی پیام باید با حساب بلاگر یا کسب\u200cوکار وارد شوید.'
                : 'This mode only previews the chat UI. Sign in with a real blogger or business account for live sending and receiving.'}
            </p>
          </div>
        ) : !adminId ? (
      ...
      <div className="p-4 border-t border-border/50">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            placeholder={
              !canSendMessages
                ? (lang === 'fa' ? 'ارسال پیام فقط با ورود واقعی فعال می\u200cشود' : 'Sign in with a real account to send messages')
                : (lang === 'fa' ? 'پیام به تیام...' : 'Message Tiam...')
            }
          ...
          <motion.button
            whileTap={{ scale: 0.95 }}
            onClick={handleSend}
            disabled={sending || (!newMsg.trim() && !attachPreview) || !canSendMessages}
